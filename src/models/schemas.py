...
def query_predictions(month: Optional[int], location: Optional[str], db: Session) -> Dict[str, list[Predictions]]:
    
    if location and month:
        return  (
            {"predictions": db.query(Predictions).filter(Predictions.location == location, Predictions.month == month).all()}
        )
    elif location is None and month:
         return  (
            {"predictions": db.query(Predictions).filter(Predictions.month == month).all()}
        )
    elif location and month is None:
         return  (
            {"predictions": db.query(Predictions).filter(Predictions.location == location).all()}
        )
    else: 
         return  (
...
def query_recommendation(month: int, location: str, activity: str, db: Session) -> Optional[RecommendationModel]: 
    ...
    recommendation_str = generate_vacation_idea_chain(month, location, days_rained,days_cloudy, days_sunny, crowded, activity)
